        ...
        if (ctx.message.reply_to_message !== undefined && admins.indexOf(ctx.from.id) > -1) {
            try {
                ...
                if (ctx.message.text === "me") { // accept ticket
                    bot.telegram.sendMessage(config.staffchat_id, "<b>" + config.lang_ticket + " #" + userid[1] + "</b> " + config.lang_acceptedBy + " " + ctx.message.from.first_name + " -> /open", cache.noSound)
                } else {
                    cache.ticketStatus[userid[1]] = false
                    bot.telegram.sendMessage(userid[1], config.lang_dear + " <b>" + name[1] + "</b>,\n\n" + ctx.message.text + "\n\n" + config.lang_regards + "\n" + ctx.message.from.first_name, cache.html)
                    console.log("Answer: " + config.lang_ticket + " #" + cache.tickedID + " " + config.lang_dear + " " + name[1] + " " + ctx.message.text + " " + config.lang_from + " " + ctx.message.from.first_name)
                }
...
function customerChat(ctx, bot, chat) {
    ...
    userInfo = ""
    userInfo += "</b> " + config.lang_from + " " + ctx.message.from.first_name + " "
    ...
    if (cache.ticketSent[cache.tickedID] === undefined) {
        ...
        bot.telegram.sendMessage(config.staffchat_id, "<b>" + config.lang_ticket + " #" + cache.tickedID + userInfo + ctx.message.text, cache.html)
        // wait 5 minutes before this message appears again and don"t send notificatoin sounds in that time to avoid spam 
        setTimeout(function () {
            cache.ticketSent[cache.tickedID] = undefined;
        }, config.spam_time)
        ...
        bot.telegram.sendMessage(config.staffchat_id, "<b>" + config.lang_ticket + " #" + cache.tickedID + userInfo + ctx.message.text, cache.noSound)
    } else if (cache.ticketSent[cache.tickedID] === 5) {
        cache.ticketSent[cache.tickedID]++;
...
function videoHandler(bot, ctx) {
    forwardFile(bot, ctx, function (userInfo) {
        bot.telegram.sendVideo(config.staffchat_id, ctx.message.video.file_id, {
            caption: config.lang_ticket + ": #" + cache.ticketID + "\n" + userInfo + "\n" + (ctx.message.caption || ""),
        })
    })
}
...
function documentHandler(bot, ctx) {
    forwardFile(bot, ctx, function (userInfo) {
        bot.telegram.sendDocument(config.staffchat_id, ctx.message.document.file_id, {
            caption: config.lang_ticket + ": #" + cache.ticketID + "\n" + userInfo + (ctx.message.caption || ""),
        })
    })
}
